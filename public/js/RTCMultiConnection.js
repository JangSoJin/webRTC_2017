(function() {
    ...
    //URL의 주소값을 받아옴
    window.URLchannel = location.href.replace(/\/|:|#|\?|\$|\^|%|\.|`|~|!|\+|@|\[|\||]|\|*. /g, '').split('\n').join('').split('\r').join('');
    ...
    window.RTCMultiConnection = function(channel) {
        ...
        connection.channel = channel || URLchannel;	//URL의 주소값을 받아서 채널링 시작.
        ...
        connection.open = function(args) {
            ...
            connection.isInitiator = true;	//메인페이지에서 start를 누르는 사람이 initiator 즉, 방장이 됨.
            ...
            return connection.sessionDescription;	//세션에 대한 설정 명세 값들을 반환함.
        };
        ...
        connection.send = function(data, _channel) {
            ...
            if (!isNull(data.size) && !isNull(data.type)) {
                if (!connection.enableFileSharing) {
                    throw '파일 공유하기 위해선 "enableFileSharing" 을 true 로 줘야함.';
                }
        ...
        function joinSession(session, joinAs) {
            ...
            // connection.join('sessionid'); 으로 들어갈 때
            if (isString(session)) {
    ...
    function setDefaults(connection) {
        ...
        connection.onstreamid = function(event) {
            ...
            log(event.userid,'로 부터 ',event.streamid,'를 받았습니다');
        };
